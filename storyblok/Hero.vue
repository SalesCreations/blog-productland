<template>
  <div v-editable="blok" id="hero-one" class="bg-gray-50 relative py-32">
    <div class="container mx-auto px-4 grid gap-5 grid-cols-12" ref="parallaxRef">
      <img src="@/assets/images/VisualElements.svg" class="w-full absolute right-0 -top-4 z-0" alt="" ref="imageRef" :style="layer0">
      
      <div class="hero-content col-start-3 col-span-8 text-center z-10 lg:px-24" ref="textRef" :style="layer1">
        <h1 class="font-display text-5xl font-black tracking-tight text-gray-900 sm:text-5xl">{{ blok.title }}</h1>
        <p class="mt-6 text-lg leading-8 text-gray-600">{{ blok.description }}</p>
      </div>

      <div class="hero-content col-span-12 mt-32 z-20" ref="photoRef" :style="layer2">
        <img src="@/assets/images/NavbarPhotos@2x.png" class="w-full"  alt="">
      </div>
    </div>
  </div>
</template>
