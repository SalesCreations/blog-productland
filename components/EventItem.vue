<template>
  <div class="col-span-12 xl:col-span-10 xl:col-start-2">
    <a :href="event.link.url" target="_blank">
      <div 
      ...
      >
        ...
        <div class="relative m-0 w-1/4 shrink-0 overflow-hidden rounded-xl rounded-l-none bg-brand-300 bg-clip-border text-white">
          <div class="date-info flex flex-col justify-center items-center h-full p-5">
            <time class="text-3xl font-black">{{ $dayjs(event.date).format('MMM').toUpperCase()}}</time>
            <time class="text-5xl font-black">{{ $dayjs(event.date).format('DD').toUpperCase()}}</time>
          </div>
        </div>
      </div>
    </a>
